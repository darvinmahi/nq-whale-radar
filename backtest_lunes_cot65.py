"""
backtest_lunes_cot65.py
Backtest especifico: LUNES + COT Index > 65% -> SHORT en NY open

Combina dos filtros:
  1. Dia de la semana = LUNES
  2. COT Index > 65% (Lev Money muy largo = Dealers presionan a la baja)

Estrategia de entrada:
  - Esperar rally en la primera media hora (9:30 - 10:00am)
  - SHORT cuando el precio toca el maximo de los primeros 30 min
  - Stop: maximo de 9:30-10:00 + 30 pts buffer
  - Target: minimo de la sesion Asia/London O -1% desde entrada
...
for d in sorted(by_date.keys()):
    ...
    if not bars_930_1000 or not bars_1000_1100:
        all_mondays.append({"date": d, "cot_idx": cot["cot_idx"], "reason": "sin datos", "skipped": True})
        continue
    ...
    # Entrada: SHORT cuando precio llega al high de los primeros 30 min
    # En backtest simplificado: entry = high_0930 (asumimos que llega)
    # En trading real: esperarías que el precio retroceda HASTA ese nivel

    ESCENARIO_A = {
        "entry"  : high_0930,
        "stop"   : high_0930 + 30,          # stop 30 pts arriba
        "target" : high_0930 - 150,         # target 150 pts abajo
        "exit"   : close_1100,              # exit forzado a las 11am
    }

    # PnL real hasta 11am (SHORT = entry - exit)
    pnl_pts = ESCENARIO_A["entry"] - ESCENARIO_A["exit"]
    win     = pnl_pts > 0
    ...
    trades.append({
        ...
        "entry"    : high_0930,
        "exit"     : close_1100,
        "move_930" : round(high_0930 - open_930, 0),   # rally previo a la entrada
...
print()
print("=" * 70)
print("  BACKTEST: LUNES + COT Index > 65%  |  SHORT en NY Open (9:30-11am)")
print("=" * 70)
print()
print("  Todos los lunes analizados: {}".format(len(all_mondays)))
print("  Lunes con COT > 65%      : {}  (filtro aplicado)".format(len(trades)))
print()
...
if trades:
    ...
    print()
    print("  DETALLE DE CADA LUNES COT > 65%:")
    print("  {:<12} {:>7} {:>10} {:>8} {:>8} {:>8} {:>8} {:>10}".format(
          "Fecha", "COTIdx", "Open9:30", "Rally", "Entry", "Exit11am", "Pts", "USD"))
    print("  " + "-" * 75)
    cum = 0
    for t in trades:
        cum += t["pnl_pts"]
    ...
    print("  - El lunes NY open sube los primeros 15-30 min (trampa de compradores)")
    print("  - Cuando el precio llega al MAXIMO de esa media hora -> SHORT")
    print("  - Stop: 30 puntos por encima del maximo")
    print("  - Exit: 11:00am o si llega al target de 150 pts")

print()
print("  TODOS LOS LUNES (con y sin filtro COT):")
