#!/usr/bin/env python3
"""
Genera martes_chart_20260318.html con VELAS REALES de NQ=F (2026-03-18)
...
# ── Descargar datos ────────────────────────────────────────────────────────
print("Descargando NQ=F 5min para 2026-03-18...")
df = yf.download("NQ=F", start="2026-03-18", end="2026-03-19",
...
# Timestamps ET para Martes 2026-03-18 (EDT = UTC-4)
# Asia Martes empieza el Lunes 17 a las 6PM ET = 10PM UTC
ASIA_START_UTC   = int(datetime(2026,3,17,22,0, tzinfo=timezone.utc).timestamp())
LONDON_START_UTC = int(datetime(2026,3,18, 6,0, tzinfo=timezone.utc).timestamp())  # 2AM ET
NY_OPEN_UTC      = int(datetime(2026,3,18,13,30,tzinfo=timezone.utc).timestamp())  # 9:30AM ET
NY_CLOSE_UTC     = int(datetime(2026,3,18,20,0, tzinfo=timezone.utc).timestamp())  # 4PM ET
...
<head>
  ...
  <title>NQ — Martes 2026-03-18 | DATOS REALES</title>
...
<div class="top-bar">
  <div style="display:flex;align-items:center;gap:12px">
    ...
    <span style="color:var(--muted);font-size:13px;font-family:'JetBrains Mono',monospace">2026-03-18 · MARTES · 5 MIN</span>
...
<div class="page">
  ...
  <div class="date-header">
    <div>
      <div style="font-size:20px;font-weight:900;margin-bottom:4px">📅 Martes 18 de Marzo 2026 — Sesión Real NQ Futures</div>
      <div style="color:var(--muted);font-size:12px">🌏 Asia (6PM ET Lun) → 🇬🇧 London 2AM → 🗽 NY Open 9:30AM → 🏁 Cierre 4PM ET</div>
    </div>
  ...
  <!-- ICT MARTES SETUP -->
  <div class="ict-box">
    <div class="ict-title">🎯 Setup ICT — MARTES COT &gt; 65% | Bear Trap + LONG Reversal</div>
    <div class="ict-grid">
      <div class="ict-item"><strong>Lógica:</strong> Martes NY open cae los primeros 30 min barriendo liquidez de longs → reversal al alza</div>
  ...
  <!-- CHART -->
  <div class="chart-section-label">📊 Chart — 5 Min · Día Completo (2026-03-18)</div>
  <div class="chart-frame">
  <div class="chart-wrap">
    <div class="chart-top">
      <span class="chart-title">NQ FUTURES — 5 MIN — 2026-03-18 MARTES · {n_candles} VELAS REALES (NQ=F via yfinance)</span>
      <div class="leg-row">
    ...
    <div id="replay-bar">
      ...
      <div id="rp-narrative">
        ...
        <div id="rp-narrative-text">Presiona PLAY para iniciar el replay del Martes 2026-03-18</div>
  ...
  <div class="panels">
    <div class="panel">
      <div class="panel-title">📋 Timeline del Martes 2026-03-18</div>
      <div class="tl-item">
        ...
        <div>
          ...
          <div class="tl-desc">NQ inicia sesión Asia el Lunes 17 a las 6PM ET. Smart money construye posición para el Martes.</div>
        </div>
      </div>
      <div class="tl-item">
      ...
      <div class="tl-item">
        ...
        <div>
          ...
          <div class="tl-desc">Apertura NY. Distance al POC: {abs(NY_OPEN_P-POC):,.0f} pts. Dirección final: {direction}.</div>
        </div>
      </div>
      <div class="tl-item">
        <div class="tl-time">9:30+</div>
        <div>
          <span class="tl-badge badge-target">🎯 MARTES ICT</span>
          <div class="tl-event c-amber">Bear Trap → NY Low: <strong>{NY_LOW:,.2f}</strong></div>
          <div class="tl-desc">Setup: caída inicial 9:30–10:00 AM barre liquidez → LONG desde el LOW de la primera media hora.</div>
        </div>
      </div>
      <div class="tl-item">
        <div class="tl-time">9:30+</div>
...
<div class="foot">NQ Whale Radar © 2026 · Datos reales: NQ=F via yfinance · {n_candles} barras de 5 min · file: martes_chart_20260318.html</div>

<script>
// ═══════════════════════════════════════════════
// DATOS REALES DE MERCADO — NQ=F — 2026-03-18
...
series.setMarkers([
  ...
  {{ time: nyLowC.time,     position:'belowBar', color:'#ef4444', shape:'arrowDown', text:`🎯 LONG Entry (Bear Trap) ${{nyLowC.low.toLocaleString()}}` }},
...
function rpUpdateUI(i) {{
  ...
  document.getElementById('rpTimeDisplay').textContent = `Mar 18  ${{h}}:${{m}} ET  C=${{c.close.toLocaleString()}}`;
...
output_file = "martes_chart_20260318.html"
