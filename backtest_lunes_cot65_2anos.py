"""
backtest_lunes_cot65_2anos.py
Descarga NQ=F 1h (~2 years) y hace backtest completo:
  LUNES + COT > 65% → SHORT entry en high de primeros 30 min
"""
...
for d, bars in by_date.items():
    ...
    ob = open_bars[0]
    # Si hay 2 barras en ny_hr, el high de la media hora son ambas
...
for d in sorted(valid_days.keys()):
    ...
    v        = valid_days[d]
    cot_idx  = cot["cot_idx"]
    rally    = v["high30"] - v["open930"]

    all_mon.append({"date": d, "cot_idx": cot_idx})

    if cot_idx <= 65:
        continue

    # Entry = SHORT en high de la primera hora
    entry   = v["high30"]
    exit_p  = v["close1100"]
...
print()
print("=" * 72)
print("  BACKTEST LUNES + COT > 65% → SHORT NY Open | 2 Años NQ=F")
print("=" * 72)
print()
print("  Lunes totales analizados  : {}".format(len(all_mon)))
print("  Lunes con COT > 65%       : {}  ({:.0f}%)".format(total, total/len(all_mon)*100 if all_mon else 0))
print()
print("  ── RESULTADOS ──────────────────────────────────────")
...
# Sub-filtro: con rally > 80 pts
t80 = [t for t in trades if t["rally"] > 80]
if t80:
    w80  = sum(1 for t in t80 if t["win"])
    pts80 = sum(t["pnl_pts"] for t in t80)
    print("  ── CON FILTRO RALLY > 80 pts ───────────────────────")
...
print("  {:<12} {:>7} {:>8} {:>9} {:>9} {:>8} {:>10}".format(
    "Fecha","COTIdx","Rally","Entry","Exit11am","Pts","USD"))
print("  " + "-" * 70)
cum = 0
for t in trades:
    cum += t["pnl_pts"]
